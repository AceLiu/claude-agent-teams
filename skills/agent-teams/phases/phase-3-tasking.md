...
5. **跨角色沟通**：通过 `send-message.sh` 发送消息（写入 `.team/messages/`），Worker 状态由 `worker-loop.sh` 自动写入 `.team/status/*.json`，Leader 通过 `team-status.sh` 查看。
...
> M 级需求可简化为口头确认（在 task 备注或消息中说明方案即可）。
...
3. **Leader 快速审查**：
   ...
   不通过 → send-message.sh 反馈 → Dev 修改设计
   通过 → 进入 Phase 4 编码

### M 级简化

M 级需求的详细设计可简化：
- Dev 在 task 备注或消息中用 2-3 段文字说明技术方案
